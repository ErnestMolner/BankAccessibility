...
def respondWithVoice(textIn):
    # make request to google to get synthesis
    tts = gtts.gTTS(textIn, lang="ca")
...
def voiceConnection():
    ...
    try:
        ...
        if str is bytes:  # this version of Python uses bytes for strings (Python 2)
            print(u"Tu as dit {}".format(value).encode("utf-8"))
...
#While the intention is not detectet to log out it keeps going
while(response != "adeu"):
    if(firstMessage):
        #Start message
        print("Disposes de dues opcions \nRealitzar transferència\nConsulta de càrrecs")
        respondWithVoice('Hola bon dia, Disposes de dues opcions, Realitzar transferència i Consulta de càrrecs')
        firstMessage = False
    else:
