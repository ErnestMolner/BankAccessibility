...
def getConfirmationCode(data):
    ...
    andBankExtractionRegex = re.compile(r'(?<=su codigo es )\d+') #This is the pattern that represents that will allow us to extrat the text message code value

    matchingValue = andBankExtractionRegex.search(emailList['subjectIndex' + str(totalEmailNum)]) #this refeairs to the las mail message recived
